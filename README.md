...
If the built program shows a black window on a machine with no working sound output (some virtual machines), start it with `SDL_AUDIODRIVER=dummy` to run without sound, for example `SDL_AUDIODRIVER=dummy ./dist/CoinTex`.
